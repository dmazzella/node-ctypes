...
const { CDLL, WinDLL, c_bool, c_int32, c_int64, c_void, c_void_p, c_char_p, c_uint16, c_uint32, c_uint64, c_size_t, c_double } = ctypes;

console.log("=== node-ctypes Windows Test ===\n");

// Load user32.dll (WinDLL uses stdcall by default)
const user32 = new WinDLL("user32.dll");
console.log("Loaded:", user32.path);

// Set the process as DPI-aware to avoid blurry windows on high-DPI displays
try {
  // Try the simpler method first: SetProcessDPIAware() - available since Windows Vista
  const SetProcessDPIAware = user32.func("SetProcessDPIAware", c_bool, []);
  if (SetProcessDPIAware()) {
    ...
    try {
      const IsProcessDPIAware = user32.func("IsProcessDPIAware", c_bool, []);
...
//
// hWnd: handle to the parent window (NULL = 0 for none)
// lpText: message text
// lpCaption: window title
// uType: button/icon type
//
// Return values:
...
const MessageBoxA = user32.func(
  "MessageBoxA",
  c_int32, // return type
  [c_void_p, c_char_p, c_char_p, c_uint32], // arg types: hWnd, lpText, lpCaption, uType
);
...
let result = MessageBoxA(null, "Hello from node-ctypes!", "Test 1 - OK", MB_OK | MB_ICONINFORMATION);
console.log("Result:", result, "(1 = OK clicked)\n");

// Test 2: Yes/No dialog
console.log("Test 2: Showing Yes/No dialog...");
result = MessageBoxA(null, "Do you like node-ctypes?", "Test 2 - Yes/No", MB_YESNO | MB_ICONQUESTION);
console.log("Result:", result, "(6 = Yes, 7 = No)\n");

// Test 3: Warning with OK/Cancel
console.log("Test 3: Showing warning dialog...");
result = MessageBoxA(null, "This is a warning message!", "Test 3 - Warning", MB_OKCANCEL | MB_ICONWARNING);
console.log("Result:", result, "(1 = OK, 2 = Cancel)\n");

// Bonus: Beep!
console.log("Bonus: Playing system beep...");
const kernel32 = new WinDLL("kernel32.dll");
const Beep = kernel32.func("Beep", c_bool, [c_uint32, c_uint32]);
...
console.log("SetLastError / GetLastError test:");
const SetLastError = kernel32.func("SetLastError", c_void, [c_uint32]);
const GetLastError = kernel32.func("GetLastError", c_uint32, []);
SetLastError(126);
console.log("  After SetLastError(126):", GetLastError());
SetLastError(0);
console.log("  Initial GetLastError():", GetLastError());
SetLastError(42);
console.log("  After SetLastError(42):", GetLastError());
...
console.log("  sizeof(c_int32):", c_int32.size);
console.log("  sizeof('int64'):", c_int64.size);
console.log("  sizeof(c_void_p):", c_void_p.size);
console.log("  sizeof(c_double):", c_double.size);
...
if (msvcrt) {
  ...
  try {
    const abs = msvcrt.func("abs", c_int32, [c_int32]);
...
if (msvcrt) {
  ...
  try {
    const strlen = msvcrt.func("strlen", c_size_t, [c_char_p]);
...
if (msvcrt) {
  ...
  try {
    const sqrt = msvcrt.func("sqrt", c_double, [c_double]);
...
try {
  const buf = ctypes.create_string_buffer(16);
  console.log("  Allocated 16 bytes");

  // Scrivi un int32
  ctypes.writeValue(buf, c_int32, 12345, 0);
  const readInt = ctypes.readValue(buf, c_int32, 0);
  console.log("  Wrote int32 12345, read back:", readInt);

  // Scrivi un double
  ctypes.writeValue(buf, c_double, 3.14159, 8);
  const readDouble = ctypes.readValue(buf, c_double, 8);
...
try {
  const buf = ctypes.create_string_buffer("Hello from Windows!");
  console.log("  Created C string");

  const str = ctypes.string_at(buf);
...
try {
  const GetComputerNameA = kernel32.func("GetComputerNameA", c_bool, [c_void_p, c_void_p]);

  // Alloca buffer per il nome e la size
  const bufSize = 256;
  const nameBuf = ctypes.create_string_buffer(bufSize);
  const sizeBuf = ctypes.create_string_buffer(4);
  ctypes.writeValue(sizeBuf, c_uint32, bufSize, 0);

  const success = GetComputerNameA(nameBuf, sizeBuf);

  if (success) {
    const computerName = ctypes.string_at(nameBuf);
...
try {
  ...
  const simpleCallback = ctypes.callback(simpleFn, c_int32, [c_int32, c_int32]);

  console.log("  Created callback");
  console.log("  Callback pointer:", "0x" + simpleCallback.pointer.toString(16));
...
if (msvcrt) {
  try {
    const qsort = msvcrt.func("qsort", c_void, [
      c_void_p, // base - array
      c_size_t, // num - numero elementi
      c_size_t, // size - dimensione elemento
      c_void_p, // compare - funzione callback
    ]);
    ...
    const arrBuf = ctypes.create_string_buffer(bufSize);

    // Copia i valori nel buffer
    for (let i = 0; i < arr.length; i++) {
      ctypes.writeValue(arrBuf, c_int32, arr[i], i * 4);
    }
    ...
    const compareFn = (aPtr, bPtr) => {
      try {
        compareCallCount++;
        const a = ctypes.readValue(aPtr, c_int32, 0);
        const b = ctypes.readValue(bPtr, c_int32, 0);
    ...
    const compareCallback = ctypes.callback(compareFn, c_int32, [c_void_p, c_void_p]);
    ...
    const sorted = [];
    for (let i = 0; i < arr.length; i++) {
      sorted.push(ctypes.readValue(arrBuf, c_int32, i * 4));
    }
...
try {
  ...
  const GetWindowTextA = user32.func("GetWindowTextA", c_int32, [c_void_p, c_void_p, c_int32]);
  ...
  const enumProc = (hwnd, lParam) => {
    ...
    const titleBuf = ctypes.create_string_buffer(256);
    const len = GetWindowTextA(hwnd, titleBuf, 256);

    if (len > 0) {
      const title = ctypes.string_at(titleBuf);
  ...
  const enumCallback = ctypes.callback(enumProc, c_int32, [
    c_void_p, // HWND
    c_void_p, // LPARAM
  ]);

  // BOOL EnumWindows(WNDENUMPROC lpEnumFunc, LPARAM lParam)
  const EnumWindows = user32.func("EnumWindows", c_bool, [c_void_p, c_void_p]);
...
if (msvcrt) {
  try {
    ...
    const arrBuf = ctypes.create_string_buffer(arr.length * 4);
    for (let i = 0; i < arr.length; i++) {
      ctypes.writeValue(arrBuf, c_int32, arr[i], i * 4);
    }
...
try {
  ...
  const threadProcFn = (param) => {
    const value = ctypes.readValue(param, c_uint32);
  ...
  const ThreadProc = ctypes.threadSafeCallback(threadProcFn, c_uint32, [c_void_p]);
  ...
  const CreateThread = kernel32.func("CreateThread", c_void_p, [
    ...
    c_uint64, // dwStackSize
    c_void_p, // lpStartAddress
    c_void_p, // lpParameter
    c_uint32, // dwCreationFlags
    c_void_p, // lpThreadId
  ]);

  // Alloca parametro
  const param = ctypes.create_string_buffer(4);
  ctypes.writeValue(param, c_uint32, 12345);

  console.log("  Parameter:", ctypes.readValue(param, c_uint32));
  ...
  // WaitForSingleObject
  const WaitForSingleObject = kernel32.func("WaitForSingleObject", c_uint32, [c_void_p, c_uint32]);
  ...
  // CloseHandle
  const CloseHandle = kernel32.func("CloseHandle", c_int32, [c_void_p]);
  ...
  setTimeout(() => {
    ...
    try {
      ...
      const GetSystemInfo = kernel32.func("GetSystemInfo", c_void, [c_void_p]);
    ...
    try {
      ...
      const GetSystemTime = kernel32.func("GetSystemTime", c_void, [c_void_p]);
      ...
      console.log("  Local Time:", `${local.wYear}-${local.wMonth}-${local.wDay} ${local.wHour}:${local.wMinute}:${local.wSecond}`);
    ...
    if (msvcrt) {
      try {
        ...
        const fibCallback = ctypes.callback(fibCompare, c_int32, [c_void_p, c_void_p]);
        const qsort = msvcrt.func("qsort", c_void, [c_void_p, c_size_t, c_size_t, c_void_p]);
    ...
    try {
      ...
      const numRects = 5;
      const rectArrayBuf = ctypes.create_string_buffer(RECT.size * numRects);
      ...
      for (let i = 0; i < 3; i++) {
        ...
        ctypes.writeValue(rectArrayBuf, c_int32, rects[i].left, baseOffset + 0);
        ctypes.writeValue(rectArrayBuf, c_int32, rects[i].top, baseOffset + 4);
        ctypes.writeValue(rectArrayBuf, c_int32, rects[i].right, baseOffset + 8);
        ctypes.writeValue(rectArrayBuf, c_int32, rects[i].bottom, baseOffset + 12);
      }
      ...
      for (let i = 0; i < 3; i++) {
        const baseOffset = i * RECT.size;
        const left = ctypes.readValue(rectArrayBuf, c_int32, baseOffset + 0);
        const top = ctypes.readValue(rectArrayBuf, c_int32, baseOffset + 4);
        const right = ctypes.readValue(rectArrayBuf, c_int32, baseOffset + 8);
        const bottom = ctypes.readValue(rectArrayBuf, c_int32, baseOffset + 12);

        const width = right - left;
        const height = bottom - top;
        console.log(`    [${i}] (${left},${top})-(${right},${bottom}) ${width}x${height}`);

        if (left !== rects[i].left || top !== rects[i].top || right !== rects[i].right || bottom !== rects[i].bottom) {
          allCorrect = false;
        }
      }
    ...
    try {
      ...
      const ptrSize = c_void_p.size;
      const argvBuf = ctypes.create_string_buffer(ptrSize * (strings.length + 1));

      const stringBufs = [];
      for (let i = 0; i < strings.length; i++) {
        const strBuf = ctypes.string_at(strings[i]);
        stringBufs.push(strBuf);
        ctypes.writeValue(argvBuf, c_void_p, strBuf, i * ptrSize);
      }

      ctypes.writeValue(argvBuf, c_void_p, 0n, strings.length * ptrSize);

      console.log("  argv simulation:");
      for (let i = 0; i < strings.length; i++) {
        const ptrToStr = ctypes.readValue(argvBuf, c_void_p, i * ptrSize);
        const str = ctypes.string_at(ptrToStr);
    ...
    if (msvcrt) {
      try {
        ...
        for (let test = 0; test < testCount; test++) {
          ...
          const callback = ctypes.callback(compareFn, c_int32, [c_void_p, c_void_p]);
    ...
    // Note: The hybrid system implemented supports:
    // - Main-thread callbacks (qsort): FunctionReference (direct, fast)
    // - External-thread callbacks (CreateThread): ThreadSafeFunction (safe)
    // The system automatically detects the calling thread and uses the appropriate mechanism.
