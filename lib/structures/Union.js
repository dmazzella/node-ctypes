...
export function createUnionClass(Structure, union, bitfield) {
  ...
  class Union extends Structure {
    static _buildStruct() {
      ...
      const def = union(mapFields, options);
      this._structDef = def;
      return def;
    }
  }
   ...
   * Python ctypes parity: `ctypes.BigEndianUnion`.
   */
  class BigEndianUnion extends Union {
    static _swapped_ = true;
  }

  /**
   * Union subclass with explicit little-endian layout (same as plain Union on
   * LE hosts). Exists for API symmetry.
   * Python ctypes parity: `ctypes.LittleEndianUnion`.
   */
  class LittleEndianUnion extends Union {
    static _swapped_ = false;
  }

  return { Union, BigEndianUnion, LittleEndianUnion };
}
