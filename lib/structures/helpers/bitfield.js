...
export function _readBitField(buf, offset, baseType, bitOffset, bitSize, sizeof) {
  ...
  // Parity Python ctypes: base type signed → sign-extend. `c_int8, 3`
  // con valore -1 (= 0b111 nei 3 bit) deve rileggere -1, non 7.
  const isSigned = baseType?._signed === true;

  if (baseSize === 8) {
    const mask = (1n << BigInt(bitSize)) - 1n;
    const raw = (value >> BigInt(bitOffset)) & mask;
    if (isSigned) {
      const signBit = 1n << BigInt(bitSize - 1);
      const extend = ~mask;
      const signed = (raw & signBit) ? (raw | extend) : raw;
  ...
  const raw = (value >> bitOffset) & mask;
  if (isSigned) {
    const signBit = 1 << (bitSize - 1);
    const extend = ~mask;
    return (raw & signBit) ? (raw | extend) : raw;
  }
  return raw;
}
