...
import { buildWrapper } from "./wrap.js";
...
export function struct(fields, options = {}, sizeof, alloc, readValue, writeValue, _isStruct, _isArrayType, _isBitField, Structure, Union, native) {
  ...
  // Python-ctypes `_pack_` semantics: numeric max alignment. Boolean `packed`
  // (no numeric pack value) means fully packed => pack = 1.
  const packValue = packed ? (typeof options.pack === "number" ? options.pack : 1) : 0;
  const packAlign = (natural) => (packValue ? Math.min(natural, packValue) : natural);
  // `swapped` selects _readerBE/_writerBE on primitive SimpleCData fields.
  // Used by BigEndianStructure / LittleEndianStructure (on LE host, swapped=true
  // means "use BE"; on theoretical BE host, same flag would mean LE — but we
  // only officially support LE hosts, so "swapped" == "big-endian").
  ...
  for (const [name, type] of Object.entries(fields)) {
    ...
    if (typeof type === "object" && type !== null && type.anonymous === true && type.type) {
      ...
      const alignment = packAlign(actualType.alignment);
    ...
    // Caso 1: Bit field
      ...
      const alignment = packAlign(Math.min(baseSize, native.POINTER_SIZE));
    ...
    // Caso 2a: Struct class (declarata come `class X extends Structure`)
      ...
      const alignment = packAlign(nestedStruct.alignment);
    ...
    // Caso 2b: Union class (declarata come `class X extends Union`)
      ...
      const alignment = packAlign(nestedUnion.alignment);
    ...
    // Caso 3: Array type
      ...
      // Array alignment == element alignment (C standard). For primitives we
      // clamp to POINTER_SIZE (same as natural alignment for all types ≤ 8
      // bytes on x64). For Structure/Union elements we must use their
      // declared alignment (smaller than size when the struct has padding).
      const elemSize = sizeof(type.elementType);
      const elemAlignment = typeof type.getAlignment === "function" ? type.getAlignment() : Math.min(elemSize, native.POINTER_SIZE);
      const alignment = packAlign(elemAlignment);
    ...
    // Caso 4b: POINTER() type — stored as pointer-sized field with typed access
      ...
      const alignment = packAlign(Math.min(size, native.POINTER_SIZE));
    ...
    // Caso 4: Tipo base (SimpleCData)
      ...
      const alignment = packAlign(naturalAlign);
  ...
  // Precalcola la lookup plana di tutti i nomi accessibili dal Proxy,
  // inclusi i sotto-campi dei campi _anonymous_. Il Proxy get/set/has
  // altrimenti scansionerebbe linearmente tutti i campi anonymous e i loro
  // subfields su ogni accesso. Usa un null-proto object per IC monomorfo.
  const anonymousFieldNames = Object.create(null);  // subField.name -> true
  for (const field of fieldDefs) {
    if (field.isAnonymous && field.type && field.type.fields) {
      for (const subField of field.type.fields) {
        anonymousFieldNames[subField.name] = true;
      }
    }
  }

  // Pre-compile reader/writer functions for each field.
  // We use null-prototype plain objects rather than `Map` because V8 builds
  // inline caches for monomorphic property accesses but not for `Map.get`,
  // and field access on a Structure Proxy is very hot. Null proto avoids
  // collisions with builtins like "constructor" or "toString".
  const fieldReaders = Object.create(null);
  const fieldWriters = Object.create(null);

  // `native.POINTER_SIZE` è una costante di processo: risolvo il branch una
  // once at reader/writer build time invece che a ogni accesso.
  const is64 = native.POINTER_SIZE === 8;

  for (const field of fieldDefs) {
    const offset = field.offset;
    const name = field.name;

    // Bitfield: pre-calcola mask/shift così read/write salta il dispatch
    // generico. Sign-extension applicata quando il base type è signed
    // (parity Python ctypes — `c_int8, 3` con valore -1 deve rileggere -1,
    // non 7).
    if (field.isBitField) {
      const fieldOffset = offset;
      const baseSize = field.baseSize;
      const bitOffset = field.bitOffset;
      const bitSize = field.bitSize;
      const isSigned = field.type?._signed === true;
      ...
      if (baseSize === 1) {
        ...
        if (isSigned) {
          ...
          fieldReaders[name] = (buf) => {
            const raw = (buf.readUInt8(fieldOffset) >> bitOffset) & mask;
            return (raw & signBit) ? (raw | extendMask) : raw;
          };
        } else {
          fieldReaders[name] = (buf) => (buf.readUInt8(fieldOffset) >> bitOffset) & mask;
        }
        fieldWriters[name] = (buf, val) => {
          const v = buf.readUInt8(fieldOffset);
          buf.writeUInt8(((v & clearMask) | ((val & mask) << bitOffset)) & 0xff, fieldOffset);
        };
      } else if (baseSize === 2) {
        const mask = (1 << bitSize) - 1;
        const clearMask = ~(mask << bitOffset);
        if (isSigned) {
          ...
          const extendMask = ~mask;
          fieldReaders[name] = (buf) => {
            const raw = (buf.readUInt16LE(fieldOffset) >> bitOffset) & mask;
            return (raw & signBit) ? (raw | extendMask) : raw;
          };
        } else {
          fieldReaders[name] = (buf) => (buf.readUInt16LE(fieldOffset) >> bitOffset) & mask;
        }
        fieldWriters[name] = (buf, val) => {
          const v = buf.readUInt16LE(fieldOffset);
          buf.writeUInt16LE(((v & clearMask) | ((val & mask) << bitOffset)) & 0xffff, fieldOffset);
        };
      } else if (baseSize === 4) {
        const mask = (1 << bitSize) - 1;
        const clearMask = ~(mask << bitOffset);
        if (isSigned) {
          ...
          const extendMask = ~mask;
          fieldReaders[name] = (buf) => {
            const raw = (buf.readUInt32LE(fieldOffset) >>> bitOffset) & mask;
            return (raw & signBit) ? (raw | extendMask) : raw;
          };
        } else {
          fieldReaders[name] = (buf) => (buf.readUInt32LE(fieldOffset) >>> bitOffset) & mask;
        }
        fieldWriters[name] = (buf, val) => {
          const v = buf.readUInt32LE(fieldOffset);
          buf.writeUInt32LE(((v & clearMask) | ((val & mask) << bitOffset)) >>> 0, fieldOffset);
        };
      } else if (baseSize === 8) {
        const bMask = (1n << BigInt(bitSize)) - 1n;
        const bBitOffset = BigInt(bitOffset);
        const bClearMask = ~(bMask << bBitOffset);
        if (isSigned) {
          const bSignBit = 1n << BigInt(bitSize - 1);
          const bExtend = ~bMask;
          fieldReaders[name] = (buf) => {
            const raw = (buf.readBigUInt64LE(fieldOffset) >> bBitOffset) & bMask;
            const signed = (raw & bSignBit) ? (raw | bExtend) : raw;
            // Ritorna Number se sta nel safe range, altrimenti BigInt
          ...
          fieldReaders[name] = (buf) => Number((buf.readBigUInt64LE(fieldOffset) >> bBitOffset) & bMask);
        }
        fieldWriters[name] = (buf, val) => {
          const v = buf.readBigUInt64LE(fieldOffset);
          buf.writeBigUInt64LE((v & bClearMask) | ((BigInt(val) & bMask) << bBitOffset), fieldOffset);
        };
      }
      continue;
    }

    // Skip complex types — gestiti dal percorso generico def.get/set
    ...
    if (field.isPointer) {
      ...
      if (is64) {
        fieldReaders[name] = (buf) => ptrType.fromAddress(buf.readBigUInt64LE(fieldOffset));
        fieldWriters[name] = (buf, val) => {
          let addr;
          if (typeof val === "bigint") addr = val;
          else if (typeof val === "number") addr = BigInt(val);
          else if (val && typeof val.address === "bigint") addr = val.address;
          else if (Buffer.isBuffer(val)) addr = val.readBigUInt64LE(0);
          else addr = 0n;
          buf.writeBigUInt64LE(addr, fieldOffset);
        };
      } else {
        fieldReaders[name] = (buf) => ptrType.fromAddress(BigInt(buf.readUInt32LE(fieldOffset)));
        fieldWriters[name] = (buf, val) => {
          let addr;
          if (typeof val === "bigint") addr = val;
          else if (typeof val === "number") addr = BigInt(val);
          else if (val && typeof val.address === "bigint") addr = val.address;
          else if (Buffer.isBuffer(val)) addr = BigInt(val.readUInt32LE(0));
          else addr = 0n;
    ...
    if (fieldType && fieldType._isSimpleCData) {
      ...
      const reader = swapped && fieldType._readerBE ? fieldType._readerBE : fieldType._reader;
      const writer = swapped && fieldType._writerBE ? fieldType._writerBE : fieldType._writer;
      fieldReaders[name] = (buf) => reader(buf, fieldOffset);
      fieldWriters[name] = (buf, val) => writer(buf, fieldOffset, val);
    }
  }
  ...
  const structDef = {
    ...
    fieldReaders,
    fieldWriters,
    anonymousFieldNames,  // usato dai nested proxy per evitare ricostruzioni
    packed: packed,
    _isStructType: true,
    ...
    create(values = {}) {
      ...
      return new Proxy(buf, buildWrapper(structDef, {
        instanceTag: "StructInstance",
      ...
      if (field) {
        ...
        if (field.isNested) {
          ...
          return field.type.create
            ? new Proxy(nestedBuf, buildWrapper(field.type, {
                instanceTag: "NestedStructInstance",
                exposeAnonymous: true,
              }))
