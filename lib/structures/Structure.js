...
export function createStructureClass(alloc, struct, bitfield, ptrToBuffer) {
  ...
  class Structure {
    constructor(...args) {
      ...
      // Lookup plana dei nomi (normali + sotto-campi anonymous) su oggetto
      // null-proto: bracket access è inline-cached da V8 (più veloce di
      // Map.has / Set.has che sono builtin megamorfici).
      const allFieldNames = Object.create(null);
      for (const field of def.fields) {
        ...
        if (field.isAnonymous && field.type && Array.isArray(field.type.fields)) {
          for (const subField of field.type.fields) {
            allFieldNames[subField.name] = true;
          }
        }
      }
      ...
      return new Proxy(this, {
        get(target, prop, receiver) {
          ...
          // `readers` può essere undefined quando questa Structure è in realtà
          // una Union (unionDef non espone fieldReaders al livello esterno).
          if (typeof prop === "string") {
            const reader = readers !== undefined ? readers[prop] : undefined;
            if (reader !== undefined) return reader(buf);
            if (allFieldNames[prop] === true) return def.get(buf, prop);
          }
          ...
          if (typeof method === "function") {
            // Bind al Proxy (receiver), NON al target raw. Un metodo custom
            // come `mag2() { return this.x * this.x + ... }` dentro fa
            // `this.x` che deve passare attraverso il Proxy get trap per
            // leggere il field. Binding su `target` (istanza raw) salta
            // il trap e ritorna undefined → NaN nei calcoli.
        ...
        set(target, prop, value, receiver) {
          // FAST PATH: pre-compiled writer (null-proto obj, bracket access)
          if (typeof prop === "string") {
            const writer = writers !== undefined ? writers[prop] : undefined;
            if (writer !== undefined) {
              writer(buf, value);
              return true;
            }
            if (allFieldNames[prop] === true) {
        ...
        has(target, prop) {
          if (typeof prop === "string" && allFieldNames[prop] === true) return true;
        ...
        getOwnPropertyDescriptor(target, prop) {
          if (typeof prop === "string" && allFieldNames[prop] === true) {
            return {
              enumerable: true,
              configurable: true,
              writable: true,
    ...
    static _buildStruct() {
      ...
      const options = {};
      if (this._pack_ !== undefined) {
        // Python ctypes: `_pack_` is a positive integer specifying the maximum
        // alignment used when laying out fields (1 = fully packed). A boolean
        // `true` is tolerated as an alias for `_pack_ = 1`. Anything else is
        // rejected so we don't silently produce a mislaid struct.
        const p = this._pack_;
        if (p === true) {
          options.pack = 1;
        } else if (p === false) {
          // explicit false = no packing, same as absent
        } else if (typeof p === "number" && Number.isInteger(p) && p >= 1 && (p & (p - 1)) === 0) {
          options.pack = p;
        } else {
          throw new TypeError(`_pack_ must be a positive power-of-two integer, got ${p}`);
        }
        if (options.pack !== undefined) options.packed = true;
      }
