 ...
 * @private
 */
export function createSimpleCDataClass(alloc, ptrToBuffer) {
  /**
  ...
  class SimpleCData {
    ...
    /**
     * Create instance viewing memory at an existing address.
     * Python ctypes parity: `T.from_address(addr)`.
     *
     * @param {bigint|number} address - Raw memory address
     * @returns {SimpleCData} Instance whose buffer aliases the given address
     */
    static from_address(address) {
      if (!ptrToBuffer) {
        throw new Error("from_address requires ptrToBuffer injected at construction");
      }
      const addr = typeof address === "bigint" ? address : BigInt(address);
      const buf = ptrToBuffer(addr, this._size);
      const inst = Object.create(this.prototype);
      inst._buffer = buf;
      return inst;
    }
    ...
    /**
     * Bind the type to an exported global variable in a shared library.
     * Reads/writes through `.value` go to/from the DLL's memory.
     *
     * Python ctypes parity: `T.in_dll(library, name)`.
     *
     * NOTE on Linux glibc: `errno` is a thread-local macro expanding to
     * `*__errno_location()`, not a real exported symbol — use `get_errno()`
     * instead for that specific case.
     *
     * @param {Object} library - CDLL/WinDLL instance (or its ._lib native handle)
     * @param {string} name - Exported symbol name
     * @returns {SimpleCData} Instance whose buffer aliases the symbol's memory
     */
