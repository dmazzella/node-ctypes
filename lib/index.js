...
const nativeModulePath = getNativeModulePath("node-ctypes", "ctypes.node");
...
function union(fields) {
  ...
  const unionDef = {
    ...
    create(values = {}) {
      ...
      // KOFFI APPROACH: ritorna plain object invece di buffer
    ...
    get(bufOrObj, fieldName) {
      ...
      // Nested struct
      if (field.isNested) {
        return field.type.toObject(buf.subarray(0, field.size));
      }

      // Array
      if (field.isArray) {
        return field.type.wrap(buf.subarray(0, field.size));
      }
    ...
    for (const field of def.fields) {
      ...
      if (!Object.prototype.hasOwnProperty.call(this, name)) {
        Object.defineProperty(this, name, {
          get: () => this._structDef.get(this._buffer, name),
          set: (v) => this._structDef.set(this._buffer, name, v),
          enumerable: true,
          configurable: true,
        });
      }
...
function struct(fields, options = {}) {
  ...
  const structDef = {
    ...
    create(values = {}) {
      ...
      // KOFFI APPROACH: converte buffer in plain object
    ...
    toObject(bufOrObj) {
      ...
      if (!Buffer.isBuffer(bufOrObj)) {
        ...
        } else if (bufOrObj && bufOrObj._structDef === structDef) {
          return bufOrObj; // Già convertito e senza _buffer? Ritorna così
        } else if (typeof bufOrObj === "object") {
          // Se è un plain object, convertilo in struct
          const buf = alloc(totalSize);
          for (const [name, value] of Object.entries(bufOrObj)) {
            structDef.set(buf, name, value);
          }
          return structDef.toObject(buf);
        } else {
          throw new TypeError("Expected Buffer or struct instance");
      ...
      // Leggi TUTTI i valori UNA VOLTA e crea plain properties
      for (const field of fieldDefs) {
        // Skip anonymous fields - i loro campi vengono esposti direttamente
        if (field.isAnonymous) {
          if (field.type && field.type.fields) {
            for (const subField of field.type.fields) {
              const subFieldName = subField.name;
              // Plain property - leggi valore UNA VOLTA
              const nestedBuf = buf.subarray(
                field.offset,
                field.offset + field.size
              );
              obj[subFieldName] = field.type.get(nestedBuf, subFieldName);
            }
          }
          continue;
        }
        ...
        if (field.isBitField) {
          ...
          value = field.type.toObject(nestedBuf);
        } else if (field.isArray) {
          const arrayBuf = buf.subarray(
            field.offset,
            field.offset + field.size
          );
          value = field.type.wrap(arrayBuf);
        } else {
          value = readValue(buf, field.type, field.offset);
        }

        // Crea plain property (non getter/setter) per massima velocità
        obj[field.name] = value;
      }

      // Aggiungi _buffer come property nascosta - accesso diretto, no sync automatico
      Object.defineProperty(obj, "_buffer", {
        value: buf,
        writable: false,
        enumerable: false,
      ...
      // Aggiungi _structDef come property nascosta
      Object.defineProperty(obj, "_structDef", {
        value: structDef,
        writable: false,
        enumerable: false,
        configurable: false,
      ...
      // Aggiungi metodo _sync() per sincronizzare manualmente le modifiche al buffer
      Object.defineProperty(obj, "_sync", {
        value: function () {
          for (const field of fieldDefs) {
            if (field.isAnonymous) {
              // Sincronizza anonymous fields
              if (field.type && field.type.fields) {
                const nestedBuf = buf.subarray(
                  field.offset,
                  field.offset + field.size
                ...
                for (const subField of field.type.fields) {
                  field.type.set(nestedBuf, subField.name, obj[subField.name]);
                }
              }
              continue;
            }
            ...
            if (field.isBitField) {
              ...
              // Nested struct - prendi il suo buffer
              if (obj[field.name] && obj[field.name]._buffer) {
                obj[field.name]._buffer.copy(buf, field.offset);
              }
            } else if (!field.isArray) {
              // Tipo base - scrivi valore
              writeValue(buf, field.type, obj[field.name], field.offset);
            }
          }
        },
        writable: false,
