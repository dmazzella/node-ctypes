...
function union(fields) {
  ...
  const unionDef = {
    ...
    get(buf, fieldName) {
      ...
      // Nested struct
      if (field.isNested) {
        return field.type.toObject(buf.slice(0, field.size));
      }

      // Array
      if (field.isArray) {
        return field.type.wrap(buf.slice(0, field.size));
      }
    ...
    set(buf, fieldName, value) {
      ...
      if (field.isNested) {
        if (Buffer.isBuffer(value)) {
          ...
          const nestedBuf = buf.slice(0, field.size);
      ...
      if (field.isArray) {
        if (Buffer.isBuffer(value)) {
          ...
          const wrapped = field.type.wrap(buf.slice(0, field.size));
    ...
    toObject(buf) {
      ...
      for (const field of fieldDefs) {
        if (field.isBitField) {
          ...
          obj[field.name] = field.type.toObject(buf.slice(0, field.size));
