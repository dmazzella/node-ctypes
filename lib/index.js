 ...
 * @param {string|CType} returnType - Tipo di ritorno
 * @param {Array<string|CType>} argTypes - Tipi degli argomenti
 * @param {Object} [options] - Opzioni: { onError: (err) => {...} }
 * @returns {Object} Oggetto con .pointer (BigInt), .release(), .lastError
 */
function callback(fn, returnType, argTypes = [], options = {}) {
  const cb = new Callback(fn, returnType, argTypes);

  // Imposta error handler se fornito
  if (options.onError && typeof options.onError === 'function') {
    cb.setErrorHandler(options.onError);
  }
  ...
  return {
    ...
    get lastError() {
      return cb.getLastError ? cb.getLastError() : null;
    },
    release() {
      cb.release();
    },
 ...
 * @returns {Object} Oggetto con .pointer (BigInt), .release(), .lastError
 */
function threadSafeCallback(fn, returnType, argTypes = [], options = {}) {
    ...
    toObject(buf) {
      ...
      for (const field of fieldDefs) {
        if (field.isBitField) {
          ...
          obj[field.name] = field.type.toObject(buf.subarray(0, field.size));
        } else if (field.isArray) {
          // Per toObject, converti array in array JS
          const wrapped = field.type.wrap(buf.subarray(0, field.size));
          obj[field.name] = [...wrapped];
        } else {
