...
import { FormatError as _FormatError, WinError as _WinError } from "./platform/errors.js";
...
import { createWinTypes } from "./platform/win_types.js";
 ...
 */
// Create Library classes (FunctionWrapper, CDLL, WinDLL, OleDLL)
const { FunctionWrapper, CDLL, WinDLL, OleDLL } = createLibraryClasses(Library, LRUCache, _toNativeType, _toNativeTypes, native);
...
// ============================================================================
// Memory Operations
// Factory con late-binding di Structure/Union (vengono creati più sotto).
// Il factory ritorna closures che leggono `memoryDeps.Structure` / `.Union`
// ad ogni call, quindi è sicuro istanziarlo prima che Structure/Union
// esistano — purché nessuno invochi readValue/writeValue/sizeof prima.
// ============================================================================

const memoryDeps = { ptrToBuffer, Structure: undefined, Union: undefined };
const { readValue, writeValue, sizeof } = createMemoryOps(memoryDeps);
 ...
 */
function union(fields, options) {
  return _union(fields, options, sizeof, alloc, readValue, writeValue, _isStruct, _isArrayType, _isBitField, Structure, Union, native);
}
...
const { Structure, BigEndianStructure, LittleEndianStructure } = createStructureClass(alloc, struct, bitfield, ptrToBuffer);
const { Union, BigEndianUnion, LittleEndianUnion } = createUnionClass(Structure, union, bitfield);

// Late-bind delle classi sul deps object delle memory ops (vedi commento
// alla creazione di memoryDeps sopra).
memoryDeps.Structure = Structure;
memoryDeps.Union = Union;
...
// get_errno / set_errno / GetLastError / SetLastError — parity Python ctypes.
// Tutte e 4 leggono/scrivono SOLO lo slot thread-local privato mantenuto dal
// native addon. Lo slot viene aggiornato da FFIFunction::Call solo quando la
// library è stata aperta con use_errno / use_last_error: true. Se non hai mai
// usato quella flag, lo slot è 0 — stesso comportamento di ctypes.get_errno()
// in Python.
//
// Per leggere il valore di sistema "grezzo" basta aprire una library qualsiasi
// con use_last_error: true e fare la call — oppure, come in Python, chiamare
// kernel32.GetLastError / msvcrt._get_errno manualmente come FFI call.

function get_errno() {
  return native.getCapturedErrno();
}

function set_errno(value) {
  return native.setCapturedErrno(value);
}

function GetLastError() {
  return native.getCapturedLastError();
}

function SetLastError(code) {
  return native.setCapturedLastError(code);
}

function FormatError(code) {
  // Parity Python: se code non specificato, usa lo slot privato
  // (ctypes.get_last_error()), non il GetLastError() di sistema.
  if (code === undefined) code = native.getCapturedLastError();
...
const SimpleCData = createSimpleCDataClass(alloc, ptrToBuffer);
...
// Windows API type aliases (Python ctypes.wintypes parity).
// Factory coerente col resto della lib: cattura primitive già create.
const wintypes = createWinTypes(primitiveTypes);

// HRESULT: c_int32 subclass tagged for OleDLL auto-error-raising.
// Python ctypes parity: `from ctypes import HRESULT`.
class HRESULT extends c_int32 {
  static _isHResult = true;
}
...
// Python ctypes.util.find_library equivalent
export { find_library } from "./platform/find_library.js";

// Lazy loader namespaces (Python's `ctypes.cdll.libname` / `ctypes.windll.libname`).
import { createLazyLoader } from "./core/lazy-loader.js";
export const cdll = createLazyLoader(CDLL);
export const windll = process.platform === "win32" ? createLazyLoader(WinDLL) : undefined;
export const oledll = process.platform === "win32" ? createLazyLoader(OleDLL) : undefined;
