...
const { Library, FFIFunction, Callback, ThreadSafeCallback, CType, StructType, ArrayType } = native;
...
function array(elementType, count) {
    ...
    return {
        ...
        create: (values) => {
            const buffer = values !== undefined ? nativeArray.create(values) : nativeArray.create();
            return wrap(buffer);
        },
    ...
    try {
        ...
        _win_error_funcs = {
            ...
            FormatMessageW: kernel32.func('FormatMessageW', 'uint32', [
                'uint32', 'pointer', 'uint32', 'uint32', 'pointer', 'uint32', 'pointer'
            ]),
            _lib: kernel32
        };
    } catch (e) {
...
function struct(fields, options = {}) {
    ...
    for (const [name, type] of Object.entries(fields)) {
        ...
        if (_isBitField(type)) {
            ...
            const canContinue = currentBitFieldBase === baseType &&
    ...
    const structDef = {
        ...
        get(buf, fieldName) {
            ...
            const field = fieldDefs.find(f => f.name === firstPart);
            if (!field) throw new Error(`Unknown field: ${firstPart}`);
        ...
        toObject(buf) {
            ...
            for (const field of fieldDefs) {
                if (field.isBitField) {
                    obj[field.name] = _readBitField(buf, field.offset, field.type, field.bitOffset, field.bitSize);
