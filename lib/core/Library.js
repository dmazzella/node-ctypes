...
export function createLibraryClasses(Library, LRUCache, _toNativeType, _toNativeTypes, native) {
  ...
  class CDLL {
    ...
    func(name, returnType, argTypes = [], options = {}) {
      ...
      // Inline-expanded `_as_parameter_` unwrap. Cheap to inline (three checks,
      // short-circuits on primitives) and avoids a function-call per arg in the
      // hot path.
      const UNWRAP = (a) => (a !== null && typeof a === "object" && a._as_parameter_ !== undefined ? a._as_parameter_ : a);
      ...
      if (argCount === 0) {
        ...
        // protocol (Python ctypes parity) but specialize wrappers per-arity
        // so V8 can inline the args instead of allocating a rest-params array.
        // Arities 1–6 cover the overwhelming majority of real-world calls.
        switch (argCount) {
          case 1:
            callMethod = function (a0) {
              return ffiFunc.call(UNWRAP(a0));
            };
            break;
          case 2:
            callMethod = function (a0, a1) {
              return ffiFunc.call(UNWRAP(a0), UNWRAP(a1));
            };
            break;
          case 3:
            callMethod = function (a0, a1, a2) {
              return ffiFunc.call(UNWRAP(a0), UNWRAP(a1), UNWRAP(a2));
            };
            break;
          case 4:
            callMethod = function (a0, a1, a2, a3) {
              return ffiFunc.call(UNWRAP(a0), UNWRAP(a1), UNWRAP(a2), UNWRAP(a3));
            };
            break;
          case 5:
            callMethod = function (a0, a1, a2, a3, a4) {
              return ffiFunc.call(UNWRAP(a0), UNWRAP(a1), UNWRAP(a2), UNWRAP(a3), UNWRAP(a4));
            };
            break;
          case 6:
            callMethod = function (a0, a1, a2, a3, a4, a5) {
              return ffiFunc.call(UNWRAP(a0), UNWRAP(a1), UNWRAP(a2), UNWRAP(a3), UNWRAP(a4), UNWRAP(a5));
            };
            break;
          default:
            // Fallback for > 6 declared args OR variadic-trailing calls. Keep
            // args in-place so a variadic user can still append values.
            callMethod = function (...args) {
              for (let i = 0; i < args.length; i++) args[i] = UNWRAP(args[i]);
              return ffiFunc.call(...args);
            };
        }
      } else {
